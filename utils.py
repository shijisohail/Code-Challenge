...
async def fetch_with_retry(session: aiohttp.ClientSession, url: str, max_retries: int = 5) -> Optional[Dict]:
    for attempt in range(max_retries):
        try:
            ...
            wait_time = min(3 + attempt * 2, 10)  # Progressive wait for timeouts
            logger.warning(
...
async def fetch_and_transform_animals(base_url: str, animal_ids: List[int], max_concurrent: int = 10) -> List[Dict[str, Any]]:
    ...
    async with aiohttp.ClientSession(
        ...
        for i in range(0, len(animal_ids), batch_size):
            ...
            logger.info(f"Processing batch {i//batch_size + 1}: animals {i+1}-{min(i+batch_size, len(animal_ids))}")
            
            # Create tasks for this batch
            tasks = [
                fetch_single_animal(animal_id)
                for animal_id in batch_ids
            ]
    ...
    logger.info(
        f"Animal processing complete: {success_count} successful, {failure_count} failed"
    )
    
    if failed_animals:
        logger.warning(f"Failed animal IDs: {failed_animals[:10]}{'...' if len(failed_animals) > 10 else ''}")
    
    return transformed_animals

async def post_batch_with_retry(session: aiohttp.ClientSession, base_url: str, animals: List[Dict], max_retries: int = 5) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning(f"Timeout sending batch, attempt {attempt + 1}/{max_retries}")
            if attempt < max_retries - 1:
